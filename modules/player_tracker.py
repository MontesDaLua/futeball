...
class PlayerTracker:
    ...
    def track_frame(self, frame):
        results = self.model.track(frame, persist=True, device=self.device, conf=self.min_confidence, verbose=False)
        return results[0]
    ...
    def calculate_speed(self, player_id, current_pos_m, dt):
        ...
        if data["positions"]:
            ...
            dist = np.sqrt((current_pos_m[0] - prev_pos[0])**2 + (current_pos_m[1] - prev_pos[1])**2)
            speed_mps = dist / dt
    ...
    def apply_filter(self, player_id):
        if len(self.player_data[player_id]["speeds"]) > 5:
            self.player_data[player_id]["speeds"] = medfilt(self.player_data[player_id]["speeds"], kernel_size=5).tolist()
